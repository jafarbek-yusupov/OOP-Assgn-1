...
void LMS::addStudent(Student student){
    for(const auto& s : students){ if(s.getId() == student.getId()){ cout << "Student with ID " << student.getId() << " ALR EXISTSSSSSSSSS" << endl; /*check for dups*/ return;}} students.push_back(student);
}

void LMS::addCourse(Course course){
    for(const auto& c : courses){ if(c.getId() == course.getId()){ cout << "Course with ID " << course.getId() << " ALR EXISTSSSSSSS" << endl; /*check for dups*/ return;}}  courses.push_back(course);
}
